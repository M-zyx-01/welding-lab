...
function renderAllTabs(data) {
  ...
  // Energy tab
  var energyData = a.energy || {};
  var eh2 = '<h3 style="margin-bottom:8px">Energy Analysis / 能量分析</h3>';
  var bal = energyData.balance || {};
  eh2 += '<div class="stat-section"><h4>Energy Balance / 能量平衡</h4><div class="info-grid">';
  eh2 += card("Input Power / 输入功率", (bal.input_power_W || "N/A") + " W");
  eh2 += card("Workpiece Power / 工件功率", (bal.power_to_workpiece_W || "N/A") + " W");
  eh2 += card("Energy per Length / 线能量", (bal.energy_per_length_J_per_m || "N/A") + " J/m");
  eh2 += card("Melting Efficiency / 熔化效率", bal.melting_efficiency !== undefined ? (bal.melting_efficiency * 100).toFixed(1) + "%" : "N/A");
  eh2 += card("Convection Loss / 对流损失", (bal.power_convection_W || "N/A") + " W");
  eh2 += card("Radiation Loss / 辐射损失", (bal.power_radiation_W || "N/A") + " W");
  eh2 += '</div></div>';
  var pd = energyData.power_density || {};
  eh2 += '<div class="stat-section"><h4>Power Density / 功率密度</h4><div class="info-grid">';
  eh2 += card("Power Density / 功率密度", (pd.power_density_W_per_mm2 || "N/A") + " W/mm2");
  eh2 += card("Peak Density / 峰值密度", (pd.peak_power_density_W_per_mm2 || "N/A") + " W/mm2");
  eh2 += card("Energy Fluence / 能量通量", (pd.energy_fluence_J_per_mm2 || "N/A") + " J/mm2");
  eh2 += card("Spot Radius / 斑点半径", (pd.spot_radius_mm || "N/A") + " mm");
  eh2 += card("Dwell Time / 驻留时间", (pd.dwell_time_ms || "N/A") + " ms");
  eh2 += card("Regime / 模式", pd.power_regime || "N/A");
  eh2 += '</div></div>';
  var eff = energyData.efficiency || {};
  eh2 += '<div class="stat-section"><h4>Efficiency / 效率</h4><div class="info-grid">';
  eh2 += card("Specific Energy / 比能量", (eff.specific_energy_kJ_per_kg || "N/A") + " kJ/kg");
  eh2 += card("Volumetric Energy / 体积能量", (eff.volumetric_energy_J_per_mm3 || "N/A") + " J/mm3");
  eh2 += card("Energy per Meter / 米能量", (eff.energy_per_meter_kWh || "N/A") + " kWh/m");
  eh2 += card("Efficiency vs Benchmark / 效率基准比", eff.efficiency_vs_benchmark || "N/A");
  eh2 += card("Benchmark Process / 基准工艺", eff.benchmark_process || "N/A");
  eh2 += '</div></div>';
  var envi = energyData.environmental_impact || {};
  eh2 += '<div class="stat-section"><h4>Environmental Energy Impact / 环境能量影响</h4>';
  eh2 += '<div class="info-grid">';
  eh2 += card("Environment / 环境", envi.environment || "N/A");
  eh2 += card("Thermal Factor / 热导因子", envi.thermal_conductivity_factor || "N/A");
  eh2 += card("Energy Penalty / 能量代价", envi.energy_penalty || "N/A");
  eh2 += '</div>';
  eh2 += '<div style="font-size:11px;color:var(--text2);margin-top:4px">' + (envi.note || "") + '</div>';
  eh2 += '</div>';
  document.getElementById("tab-energy").innerHTML = eh2;
}
